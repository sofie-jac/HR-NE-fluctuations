...
#Contol Mice version
Control_mouse_HRB_amplitude_R_data <- read_csv("C:/Users/trb938/OneDrive - University of Copenhagen/MATLAB/Human_data/mouse/Control_mouse_HRB_amplitude_R_data.csv")
```
...
unique(filtered_data$SubjectID)

bs_human_RR_sigma <- perform_bootstrap_r2(clean_data, RR_recover_ampl  ~  sigma_bin2_raw + (1 | SubjectID), "SubjectID", R = 1000)

r.squaredGLMM(human_RR_sigma)
summary(human_RR_sigma)
...
#Outlier extraction
mean_RR_recover_ampl <- mean(Pin_Chun_data$RR_recover_ampl, na.rm = TRUE)
sd_RR_recover_ampl <- sd(Pin_Chun_data$RR_recover_ampl, na.rm = TRUE)

# Calculate the mean and standard deviation for RR_recover_AUC
mean_RR_recover_AUC <- mean(Pin_Chun_data$RR_recover_AUC, na.rm = TRUE)
sd_RR_recover_AUC <- sd(Pin_Chun_data$RR_recover_AUC, na.rm = TRUE)
...
clean_data <- Pin_Chun_data %>%
  filter(
    ...
    abs(RR_recover_AUC - mean_RR_recover_AUC) <= 2.5 * sd_RR_recover_AUC
  )


ggplot(clean_data, aes(x = RR_recover_ampl, y = sigma_bin2_raw)) +
  geom_point() +
...
ggplot(clean_data, aes(x = RR_recover_AUC, y = sigma_bin2_raw)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(
    title = "Correlation between Sigma Power (-5 to 0) and parasymp RR post-HRB",
...
write.csv(clean_data, "C:\\Users\\trb938\\OneDrive - University of Copenhagen\\MATLAB\\Human_data\\Pin-Chun_signal_corr_df.csv")

summary(lm(formula = PerChange ~  Mean_RR_recover_AUC, data = subbase_pin_chun))

ggplot(subbase_pin_chun, aes(x = Mean_RR_recover_AUC, y = PerChange)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(
    title = "Correlation between parasymp RR post-HRB and memory",
    x = "RR AUC",
    y = "% Change"
  ) +
  theme_minimal()
