...
library(pacman)
pacman::p_load(optimx, MuMIn, tidyverse, emmeans, MASS, boot, robustlmm, dplyr, lmtest, fitdistrplus, car, ggpubr, ggplot2, ggthemes, plyr, lme4, RColorBrewer, reshape2, afex, emmeans, psych, lmerTest, compare, rstatix, ggExtra, gridExtra, ggeffects, modelr, boot, stats)
...
perform_bootstrap_fixed_effects_NAs <- function(data, formula, subject_var, R = 1000) {
  # Filter out rows with NA values in the predictors
  data <- na.omit(data)
  ...
  boot_model <- function(data, indices) {
    ...
    if (inherits(model, "try-error")) {
      ...
      cor_matrix <- vcov(model) / (sqrt(diag(vcov(model))) %*% t(sqrt(diag(vcov(model)))))
      cor_fixed_effects <- cor_matrix["(Intercept)", "Delta"]
      return(cor_fixed_effects)  # Return the correlation of fixed effects
    }
  }
...
pairwise_ci_RR <- perform_bootstrap_fixed_effects_correlation(df, RR ~ NE + (NE | SubjectID), "SubjectID", R = 1000)
...
# Example initialization with a continuous predictor
ci_delta <- perform_bootstrap_fixed_effects_NAs(df, RR ~  Delta + (1|SubjectID), "SubjectID", R = 1000)
...
df$RR_inv <- 1 / (df$RR + 1)
# Fit Models with Transformations
m_h1_transformed <- lmer(formula = RR_log ~ Theta + (Theta|SubjectID), data = df, control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e5)))
m_sqrt <- lmer(RR_sqrt ~ Theta + (Theta|SubjectID), data = df, control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e5)))
m_inv <- lmer(RR_inv ~ Theta + (Theta|SubjectID), data = df, control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 1e5)))

# Check Assumptions for all transformations
check_lmm_assumptions(m_inv)

# Example initialization with a continuous predictor
ci_delta <- perform_bootstrap_fixed_effects_NAs(df, RR ~  Theta + (1|SubjectID), "SubjectID", R = 1000)
...
# Example initialization with a continuous predictor
ci_delta <- perform_bootstrap_fixed_effects_NAs(df, m_sqrt ~  Sigma + (1|SubjectID), "SubjectID", R = 1000)

summary(m_sqrt)
r.squaredGLMM(m_sqrt)

print(ci_delta)

# Extract fitted values from the model
fitted_values <- fitted(m_sqrt)
...
observed_values <- df$RR_sqrt[used_indices]
