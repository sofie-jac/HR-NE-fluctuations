 function RR_intervals = extract_RR_intervals_chr2(uniqueIDs)
    % Directories
    ...
    for idx = 1:length(uniqueIDs)
        ...
        % Get laser shift times for the current subject
        laser_shifts = laser_shift_seconds.(id_with_M);

        % Add pre and post laser periods (laser_0)
        laser_shifts = [0, laser_shifts, RR_time(end)]; %#ok<AGROW>

        % Extract RR intervals for each laser level during NREM periods
        for laser_level = 0:5
            if laser_level == 0
                ...
                for i = 1:size(NREM_periods, 1)
                    ...
                    if onset < end_time && offset > start_time
                        RR_interval = RR(RR_time >= max(onset, start_time) & RR_time <= min(offset, end_time));
                        if ~isempty(RR_interval)
                            RR_intervals = [RR_intervals; {RR_interval, uniqueId, laser_level}];
                        end
                    end
                    if onset < end_time2 && offset > start_time2
                        RR_interval = RR(RR_time >= max(onset, start_time2) & RR_time <= min(offset, end_time2));
                        if ~isempty(RR_interval)
                            RR_intervals = [RR_intervals; {RR_interval, uniqueId, laser_level}];
                        end
                    end
