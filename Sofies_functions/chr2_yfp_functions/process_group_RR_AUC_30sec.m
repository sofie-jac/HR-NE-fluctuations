function [AUC_table, PeakFreq_table, PeakPower_table] = process_group_RR_AUC_30sec(group, min_fs, max_fs)
    fs = 64;
    sampling_hz = 0.002;
    ...
    for s = 1:length(group)
        ...
        for laser_level = 1:5
            ...
            for i = 1:size(RR_values, 1)
                ...
                [pxx, f] = pwelch(bout_data, [], [], [min_fs:sampling_hz:max_fs], fs); % Calculate PSD
