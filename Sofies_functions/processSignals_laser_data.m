function [delta465_filt_2, sec_signal_2, signal_fs, EEG, EMG, sec_signal_EEG, EEG_fs, TTL_EEG_onset, laser_binary_vector, Data] = processSignals_laser_data(mouse, data_FPrig)
    ...
    laser_on = data_FPrig.epocs.(mouse{13}).onset - TTL_FP(1);
    laser_off = data_FPrig.epocs.(mouse{13}).offset - TTL_FP(1);
    
    laser_binary_vector = zeros([1, length(signal_465_2)]); 
    ...
    linkaxes([a,b,c,d],'x'); 
    e = subplot(5,1,5);
    plot(sec_signal_2, laser_binary_vector)
    ylim([-1 2])
    title('laser');
