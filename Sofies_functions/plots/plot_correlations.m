function plot_correlations(amplitude_diff_data, event_var)
        ...
        for event_type = 1:length(event_var)
            ...
            if contains(uniqueId, event_type_name)
                ...
                if isfield(amplitude_diff_data.(uniqueId), 'Gamma_low_diff')
                    ...
                    Gamma_low_diff_data = [Gamma_low_diff_data; NaN(length(amplitude_diff_data.(uniqueId).NE_diff), 1)];
                end
                if isfield(amplitude_diff_data.(uniqueId), 'Gamma_high_diff')
                    ...
                    Gamma_high_diff_data = [Gamma_high_diff_data; NaN(length(amplitude_diff_data.(uniqueId).NE_diff), 1)];
                end
    ...
    bands = {'RR', 'SO', 'Delta', 'Theta', 'Sigma', 'Beta', 'Gamma Low', 'Gamma High'};
    data = {RR_data, SO_data, Delta_data, Theta_data, Sigma_data, Beta_data, Gamma_low_diff_data, Gamma_high_diff_data};
