function auc_data = extract_auc_and_plot(o, event_var, saveDirectory, times_matrix_post, outputDirectory, titles)
    ...
    for stage_idx = 1:length(event_var)
        ...
        for idx = 1:length(o)
            ...
            RR_data = RR_collector.(['RR_' event_type_name '_' uniqueId]);
            NE_data = NE_collector.(['NE_' event_type_name '_' uniqueId]);
            if ~any(strcmp(uniqueId, {'213', '205'}))
                SO_data = SO_collector.(['SO_' event_type_name '_' uniqueId]);
                Delta_data = Delta_collector.(['Delta_' event_type_name '_' uniqueId]);
                Theta_data = Theta_collector.(['Theta_' event_type_name '_' uniqueId]);
                Sigma_data = Sigma_collector.(['Sigma_' event_type_name '_' uniqueId]);
                Beta_data = Beta_collector.(['Beta_' event_type_name '_' uniqueId]);
                Gamma_low_data = Gamma_low_collector.(['Gamma_low_' event_type_name '_' uniqueId]);
                Gamma_high_data = Gamma_high_collector.(['Gamma_high_' event_type_name '_' uniqueId]);
            end

            % Calculate AUC for each event for post time matrices
            auc_summary.NE_post = calculate_auc_from_min_max(NE_data, time_vector_NE, times_matrix_post.NE.range);
            auc_summary.RR_post = calculate_auc_from_min_max(RR_data, time_vector_RR, times_matrix_post.RR.range) * 1000;  % Convert to ms
            ...
            if ~any(strcmp(uniqueId, {'213', '205'}))
                ...
                for band_idx = 1:length(eeg_bands)
                    ...
                    auc_summary.([band_name '_post']) = calculate_auc_from_min_max(band_data, time_vector_EEG, times_matrix_post.EEG.range);
                end
            end
...
function auc = calculate_auc_from_min_max(data, time_vector, time_range)
    ...
    auc = trapz(time_vector(indices), data_segment, 2); % Use abs to ensure positive AUC
end
